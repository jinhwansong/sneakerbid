# 작업 관리

> 작업순위·추가작업·보안 항목 정리 (백엔드/프론트 분리)

---

## 📌 다음에 할 작업 (TODO)

### 🔔 알림 — 우선

- [x] 알림 API (GET /notifications, GET unread-count, PATCH read-all, PATCH /:id/read)
- [x] 알림 생성 이벤트 (낙찰, 입찰 추월) · [ ] 경매 종료 임박 *(미구현)*
- [x] 실시간 알림 (SSE `GET /notifications/stream`)
- [x] 헤더 알림(종) 아이콘, 드롭다운/패널 UI
- [x] 알림 목록 연동, 읽음·전체 읽음, 미읽음 배지
- [x] 모바일 헤더: 햄버거 메뉴 + 사이드 패널 (경매·거래내역·랭킹)

### 📚 스토리북 — 여유 있을 때

- [ ] Storybook 설치 및 설정
- [ ] 공통 컴포넌트 스토리 (Button, Badge, Input, Dropdown 등)
- [ ] 도메인 컴포넌트 스토리 (AuctionCard, MyAuctionCard 등)
- [ ] Chromatic 또는 CI 연동 *(선택)*

---

## ✅ 완료된 항목 요약

### 핵심 기능 (완료)
- 경매 등록/목록/상세, 입찰, 낙찰, 결제, 찜하기, 마이페이지
- 알림 (REST + SSE, 헤더 종·미읽음 배지, 모바일 반응형)
- 모바일 헤더 네비 (햄버거 + 사이드 패널)
- SSE 실시간 갱신 (메인, LiveStats, 거래내역)
- 관리자 페이지 (정산, 봇, 경매 강제종료, 차트)
- OAuth (Google/Kakao), JWT, Redis 리프레시

### 테스트·인프라 (완료)
- Backend: Jest 단위/E2E, Frontend: Vitest 단위, Playwright E2E
- Supabase (DB, Storage), Upstash Redis
- 404, Error Boundary, env 검증, Swagger

---

## 📂 완료 내역 (상세)

### Critical
- [x] SSE Redis Pub/Sub
- [x] payOrder 조건부 상태 전환 (이중 결제 방지)
- [x] PENDING 주문 타임아웃 Cron
- [x] 결제 실패 시 Auction REOPEN
- [x] SSE 재연결(backoff)
- [x] 입찰 실패 UX 개선

### Important
- [x] 경매 종료 전용 이벤트 (auctionClosed)
- [x] 봇 시뮬레이션 개선
- [x] EventsService 구독 0일 때 Subject 정리
- [x] closeExpiredAuctions 배치
- [x] 찜하기 API
- [x] auctionClosed 이벤트 수신·UI 반영
- [x] 낙관적 입찰, 상태 기반 UI

### 보안
- [x] payOrder 조건부 전환, Redis publish 로깅, buyNow 잔액 사전 검증

### 경매·마이페이지
- [x] 경매 등록/목록/상세, 이미지 업로드
- [x] 마이페이지 (/me, auctions, bids, wishlist), 주문 목록
- [x] 경매 수정 (/me/auctions/[id]/edit)

### 관리자
- [x] ADMIN guard, 정산 API, 봇 관리, 강제 종료, 입찰 히스토리 차트

### Polish
- [x] 종료 임박 강조, 최고 입찰자 표시, 404, Error Boundary
- [x] 메타 태그, 접근성, env 검증, API 타입 통일

### 테스트
- [x] Backend Jest + E2E, Frontend Vitest + Playwright E2E

### 인프라
- [x] Supabase (DB, Storage), Upstash Redis
- [x] KicksDB 상품 시드 Cron *(매일 00:00, 09:00, 12:00, 14:00, 18:00 — 10개씩)*
- [x] DB 보존 배치 *(매일 03:15 KST — `closedAt` 기준 N일 지난 종료 경매·관련 Order/지갑/입찰 삭제, 미참조 Sneaker 정리. `DATA_RETENTION_DAYS`, 기본 7일, 0=비활성)*

---

## 💡 추가 가능한 기능 아이디어

### 추천 (체크리스트)

- [x] **검색** — 경매 목록 `search` (브랜드·모델명·컬러웨이·설명 ILIKE)
- [x] **판매자 대시보드** — `/me/seller`, API `GET /users/me/seller-dashboard` (조회수·입찰·매출·낙찰률)
- [x] **리뷰·평점** — `POST /orders/:orderId/reviews`, `GET /users/:userId/reviews`, 결제 완료 주문에서 작성
- [x] **가격/종료 알림** — 찜: 종료 15분 전 알림(Cron) · 새 입찰 시 현재가 변동 알림(60분당 1회 스로틀)

### 보류

- 지갑 충전 *(결제 연동 복잡도)*
- 배송 흐름 *(실거래 연장선)*

**참고**: 오름 경매 특성상 “가격 하락”은 없고, 찜한 경매 **현재가 변동(신규 입찰)** 알림으로 대응했습니다.
